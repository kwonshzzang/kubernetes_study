# DNS 서버를 이용해서 Service 이름으로 Service 호추
...
# 'payment' 서비스 이름으로 order -> payment 파드 요청/응답확인
kubectl exec -it <order-pod-name> [-c <container-name] -n snackbar -- curl -s payment:80

# snackbar 네임스페이스에 있는 order 컨테이너의 /etc/hosts 확인
kubectl exec -it <order-pod-name> [-c <container_name>] -n snackbar -- cat /etc/hosts

# kube-system 네임스페이스에 있는 모든 kube-dns 리소스 조회
# -- kube-dns Deployment를 통해 kube-dns Pod를 생성, ClusterIP 타입의 kube-dns Service를 통해 Pod 집합에 대한 단일 엔드포인트
kubectl get all -n kube-system | grep kube-dns

# snackbar 네임스페이스의 order 컨테이너 쉘 접속
kubectl exec -it <order-pod-name> [-c <container_name>] -n snackbar -- sh
