...
# fancy-snackbar 네임스페이스에 Deployment로 delivery 파드 배포, deliver 파드 집합을 위한 Service 배포
kubectl apply -f source/service-of-different-namespace.yaml
...
# snackbar 네임스페이스에 order 컨테이너에서 fancy-snackbar 네임스페이스에 있는 delivery 서비스 ClusterIP로 요청 실행 후 응답확인
kubectl exec -it <order-pod-name> [-c <container_name>] -n snackbar -- curl -s 10.105.203.168

# snackbar 네임스페이스에 order 컨테이너에서 fancy-snackbar 네임스페이스에 있는 delivery 서비스 도메인 이름으로 요청 실행 후 응답 확인
kubectl exec -it <order-pod-name> [-c <container_name>] -n snackbar -- curl -s deliver.fancy-snackbar

# snackbar 네임스페이스에 order 컨테이너에서 환경변수 목록 조회 - fancy-snackbar 네임스페이스의 delivery 서비스 환경변수 확인
kubectl exec -it <order-pod-name> [-c <container_name>] -n snackbar -- env
